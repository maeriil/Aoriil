...
class textsection:
    """"""
    def __init__(self, text, position, font_size, font_type="placeholder") -> None:
        self.set_text(text)
...
class parenttextsection:
    ...
    def similar_fonts (self, section: textsection) -> bool:
        return (section.font_size in range(self.font_size - 5, self.font_size + 5))
    ...
    def draw_section (self, image: np.array) -> np.array:
        ...
        cv2.rectangle(image, self.start_pos, self.end_pos, color=border_color, thickness=border_pixel)
        return image
    ...
    def in_y_axis_range (self, section: textsection) -> bool:
        ...
        return not self.completely_above(section) and not self.completely_below(section)
    ...
    def in_x_axis_range (self, section: textsection) -> bool:
        ...
        return not self.completely_left(section) and not self.completely_right(section)
    ...
    def contains_edges_yaxis (self, section: textsection, image: np.array) -> bool:
        """"""
        ...
        cropped_img = imgutil.crop_image(image, [start_pos_x, start_pos_y], [end_pos_x, end_pos_y])
    ...
    def contains_edges_xaxis (self, section: textsection, image: np.array) -> bool:
        """"""
        start_pos_x = 0
        end_pos_x = 0
...
def should_merge_sections (parent_section: parenttextsection, child_section: textsection, MAXIMUM_GAP_DIST: int, image: np.array) -> bool:
    """
    """
    # if not parent_section.similar_fonts(child_section):
    #     return False
    if parent_section.overlaps(child_section):
        ...
        if not parent_section.in_x_axis_range(child_section):
            return False
        if parent_section.contains_edges_yaxis(child_section, image):
            return False
    elif parent_section.in_y_axis_range(child_section):
        if parent_section.x_difference(child_section) > MAXIMUM_GAP_DIST:
            return False
        if parent_section.contains_edges_xaxis(child_section, image):
            return False
    return True
...
def calculate_long_edge (image: np.array) -> bool:
    ...
    for cnt in contours:
        temp_thresh = cv2.drawContours(np.zeros_like(threshold), [cnt], -1, 255, 1)
